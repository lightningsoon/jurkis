...
def circle(img_bgr,img_4_draw):
    ...
    for cnt in contours:
        if len(cnt) > 50:
            ...
            if 0.90 < S1 / S2 < 1.1 and 0.9 > S2 / square > 0.25:
                img_4_draw=cv2.ellipse(img_4_draw,ellipse,color=(130, 80,0),thickness=2)
                cnt=np.reshape(cnt,(len(cnt),2))
...
class Where_am_I(object):
    def __init__(self,imgshape=(640,480,3)):
        self.width, self.height, self.channel = imgshape
    ...
    def generate_new_background(self):
        import numpy as np
        width,height,channel=self.width,self.height,self.channel
    ...
    def draw_grasp_point_and_arm_center(self,frame,grasp_point):
        ...
        if not np.any(np.array(grasp_point) == None):
