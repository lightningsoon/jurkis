...
def get_x(ims):
    ...
    # print(imgs.shape,imgs.dtype)#(21, 224, 224, 3)
    ...
    return features
def save_centroid_npz(centroid,K,update_npz=False,filename=(workdir+'*means.npz')):
    #  保存 centers，K
    # 如果已经保存就恢复
    ...
    if update_npz or not os.path.isfile(filename):
        ...
        K, centroid = list(npdata[na] for na in npdata.files)#数据读取顺序，可能是堆栈式的
        # print(centroid)
...
class Who_am_I(object):
    ...
    def __init__(self):
        ...
        # print(self.__cents,self.__K)
        self.__get_x=get_x
        self.__down_Dimension=downDimension
        self.__th=get_nearest_neighbor_threshold(None,self.__cents)
...
if __name__ == '__main__':
    ...
    # 多试几次，肯定能有个好点初始值
    filename1=workdir+str(K)+'means.npz'
    centroid, K=save_centroid_npz(centroid,K,False,filename=filename1)
    # print(centroid,K)
